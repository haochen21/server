...
public class NeedPayCarMonitor {
    ...
    public void start() {
        ...
        executor = Executors.newFixedThreadPool(configProperties.getNeedPayProcessNum());
        processCart();
        addNotClosedCartToQueue();
    }

    protected void processCart() {
        for (int i = 0; i < configProperties.getNeedPayProcessNum(); i++) {
